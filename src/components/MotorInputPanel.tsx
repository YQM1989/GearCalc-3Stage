...
const NumberField = ({
  ...
  <label className="grid min-w-0 gap-1">
    ...
    <div className="flex rounded-md border border-slate-300 bg-white focus-within:border-teal-600 focus-within:ring-2 focus-within:ring-teal-600/15">
      ...
      />
      <span className="flex min-w-12 items-center justify-center rounded-r-md border-l border-slate-200 bg-slate-50 px-2 text-[11px] font-medium text-slate-500">
        {unit}
      </span>
    </div>
  </label>
...
export const MotorInputPanel = ({
  ...
  <section className="rounded-lg border border-slate-200 bg-white p-3 shadow-sm">
    <div className="mb-3 flex items-start justify-between gap-3">
      <div>
        <h2 className="text-sm font-semibold text-slate-950">电机参数</h2>
        <p className="mt-0.5 text-xs text-slate-500">压力角第一版仅用于显示。</p>
      </div>
    </div>
    ...
    <div className="mt-3">
      <span className="text-xs font-medium text-slate-700">级数选择</span>
      <div className="mt-1.5 grid grid-cols-3 rounded-md border border-slate-300 bg-slate-50 p-1">
        {[1, 2, 3].map((count) => (
          <button
            key={count}
            type="button"
